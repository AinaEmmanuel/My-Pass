...
""")
    ...
    message['From'] = config('GUNMAIL_EMAIL')
    message['To'] = user.email

    server = smtplib.SMTP('smtp.mailgun.com', 587)
    # server.starttls()
    server.login(config('GUNMAIL_EMAIL'),config('GUNMAIL_PASSWORD'))
    server.sendmail(config('GUNMAIL_EMAIL'), user.email, message.as_string())
    server.quit()
...
"""
    ...
    # server.starttls()
    server.login(config('GUNMAIL_EMAIL'),config('GUNMAIL_PASSWORD'))
    server.sendmail(config('GUNMAIL_EMAIL'), user, message.as_string())
    server.quit()
