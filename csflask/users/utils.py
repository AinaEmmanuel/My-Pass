...
""")
    ...
    message['From'] = os.environ.get('GUNMAIL_EMAIL')
    message['To'] = user.email

    server = smtplib.SMTP('smtp.mailgun.com', 587)
    # server.starttls()
    server.login(os.environ.get('GUNMAIL_EMAIL'),os.environ.get('GUNMAIL_PASSWORD'))
    server.sendmail(os.environ.get('GUNMAIL_EMAIL'), user.email, message.as_string())
    server.quit()
...
"""
    ...
    # server.starttls()
    server.login(os.environ.get('GUNMAIL_EMAIL'),os.environ.get('GUNMAIL_PASSWORD'))
    server.sendmail(os.environ.get('GUNMAIL_EMAIL'), user, message.as_string())
    server.quit()
